...
- **Bending-plane orientation** $\phi$: ramp reference from a negative to a positive orientation.
```math
\bar{\phi}(t) = -3 + 6\frac{t}{t_f}.
```
- **Bending magnitude** $\theta$: sinusoidal reference.
```math
...
This figure shows the desired and actual trajectories for the bending-plane orientation $\phi$ and the bending magnitude $\theta$.

### Internal bending torques

![Internal bending torques](./figures/spatial_internal_torques.png)

This figure shows the internal bending torques $(\tau_x, \tau_y)$ generated by the actuator system. These torques correspond to the local bending directions before being mapped into curvature-space generalized efforts.
